...
STAGING_DIR = staging
DOCKER_IMAGE?=registry.millegrilles.com:5000/millegrilles/filehost_python
...
deploy: package
	...
	@for dir in catalogue/*; do \
		if [ -d "$$dir" ]; then \
			...
			echo "==> Deploying $$NAME"; \
			rsync "$(ARTIFACTS_DIR)/$$NAME.$(VERSION_FULL).tar.gz" ${DEPLOY_RSYNC_WEBAPP_DEST}/filehost/${NAME}/; \
			${DEPLOY_CATALOGUE_UPDATE_COMMAND} --baseurl https://libs.millegrilles.com/archives/filehost --archive archives/filehost/"$$NAME.$(VERSION_FULL).tar.gz"; \
		fi; \
	done
